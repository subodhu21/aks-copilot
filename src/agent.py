...
from prompt_loader import (
    ...
    build_namespace_ops_prompt,
    build_remediation_rationale_prompt,
    build_platform_advice_prompt,
    build_helm_fix_prompt,
)
...
def analyze_logs(logs, description):
    ...
    prompt = build_log_analysis_prompt(issue_type, compact_description, compact_logs)
...
def analyze_namespace_operations(snapshot):
    ...
    prompt = build_namespace_ops_prompt(snapshot)
    try:
...
def generate_remediation_rationale(plan, description_short, logs_excerpt):
    ...
    prompt = build_remediation_rationale_prompt(plan, description_short, logs_excerpt)
    try:
        return _chat_with_ai(prompt)
    except Exception:
        return fallback
...
def platform_engineering_advice(goal, snapshot, constraints=None):
    ...
    prompt = build_platform_advice_prompt(goal, constraints, snapshot)
    try:
        response = _chat_with_ai(prompt)
        return {"ai_response": response, "fallback": fallback}
    except Exception:
...
def create_fix_pr(pod_name, namespace, k8s_context=None):
    ...
    try:
        ...
        ai_prompt = build_helm_fix_prompt(
            pod_name=pod_name,
            namespace=namespace,
            ...
            helm_values_content=helm_values_content[:3000] if helm_values_content else "",
